...
export function CargoFormDialog({
  ...
  const set = <K extends keyof FormState>(key: K, value: FormState[K]) => {
    setForm((f) => {
      ...
      } else if (key === "kind" && f.kind === "cylinder") {
        next.height = "1";
      }
      return next;
    });
  ...
  const submit = () => {
    ...
    rangeChecks.length = rangeError(
      "length",
      fromLengthToMm(data.length, lengthUnit)
    );
    rangeChecks.width = rangeError(
      data.kind === "cylinder" ? "diameter" : "width",
      fromLengthToMm(data.width, lengthUnit)
    );
    rangeChecks.height = rangeError(
      "height",
      fromLengthToMm(data.height, lengthUnit)
    );
    rangeChecks.weight = rangeError(
      "weight",
      fromWeightToKg(data.weight, weightUnit)
    );
    rangeChecks.count = rangeError("count", data.count);
    rangeChecks.maxLoad = rangeError(
      "maxLoad",
      fromWeightToKg(data.maxLoad, weightUnit)
    );
